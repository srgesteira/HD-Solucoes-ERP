-- Código técnico MO: MO-[material][acabamento]-[sequencial] (ex.: MO-A10-001).
...
BEGIN
  ...
  IF v_prefix_code = 'MO' THEN
    IF NEW.material_id IS NULL OR NEW.finish_id IS NULL THEN
      RAISE EXCEPTION 'Produto MO requer material e acabamento';
    END IF;
    ...
    NEW.technical_code :=
      'MO-' || v_material_code || v_finish_code || '-' || LPAD(v_next_seq::TEXT, 3, '0');
    RETURN NEW;
  END IF;
...
-- Recodificar produtos MO legados MO-001 (sem material) que tenham material+acabamento.
DO $$
DECLARE
  r RECORD;
  v_mat TEXT;
...
BEGIN
  FOR r IN
    ...
    WHERE
      ...
      AND (
        p.technical_code ~ '^MO-[0-9]{3}$'
        OR TRIM(COALESCE(p.technical_code, '')) = ''
      )
    ORDER BY
      p.created_at,
      p.id
  LOOP
    ...
    IF v_mat IS NULL OR v_fin IS NULL THEN
      CONTINUE;
    END IF;
    ...
    WHERE
      ...
      AND p.id <> r.id
      AND p.technical_code ~ ('^MO-' || v_mat || v_fin || '-[0-9]{3}$');

    v_seq := v_seq + 1;
    v_code := 'MO-' || v_mat || v_fin || '-' || LPAD(v_seq::TEXT, 3, '0');

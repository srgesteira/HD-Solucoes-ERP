...
export type CashFlowMeta = {
  ...
  outflow_source: string;
  provisional_projection_fallback_count: number;
  provisional_projection_fallback_ids: string[];
};
...
/**
 * Projeção de fluxo de caixa: entradas (receivables) vs saídas (AP + PCs
 * confirmados sem AP + contas fixas). Extraído de /api/reports/cash-flow
 * para ser reaproveitado por outras rotas (ex.: alerta financeiro) sem
 * duplicar a lógica de merge/datação.
 */
export async function computeCashFlowProjection(
  ...
  if (companyErr) {
    throw new Error("Configurações da empresa: " + companyErr.message);
  }
  ...
  if (rErr) {
    throw new Error("Recebíveis: " + rErr.message);
  }
  ...
  if (apErr) {
    throw new Error("Contas a pagar: " + apErr.message);
  }
  ...
  if (salesOrderIds.length > 0) {
    const { data: salesOrders, error: soErr } = await admin
      ...
      .in("id", salesOrderIds);
    if (soErr) {
      throw new Error("Pedidos de venda: " + soErr.message);
    }
  ...
  if (purchaseOrderIds.length > 0) {
    ...
    if (poErr) {
      throw new Error("Pedidos de compra: " + poErr.message);
    }
  ...
  if (pErr) {
    throw new Error("Compras: " + pErr.message);
  }
  ...
  if (feErr) {
    throw new Error("Contas fixas: " + feErr.message);
  }
  ...
  if (expenseIds.length > 0) {
    ...
    if (ovErr) {
      throw new Error("Overrides de contas fixas: " + ovErr.message);
    }
  ...
  const series: CashFlowSeriesRow[] = [];

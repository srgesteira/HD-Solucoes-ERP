...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const { order, error: orderErr } = await resolvePurchaseOrder(
    admin,
    tenantId,
    orderId
  );
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const production_order_id =
    b.production_order_id === undefined || b.production_order_id === null
      ? null
      : String(b.production_order_id);

  const production_item_id =
    b.production_item_id === undefined || b.production_item_id === null
      ? null
      : String(b.production_item_id);
  ...
  if (production_order_id) {
    const { data: po } = await admin
      ...
      .eq("tenant_id", tenantId)
      .maybeSingle();
    if (!po) return apiError("Pedido de produção inválido", 400);
  }
  ...
  if (production_item_id) {
    const { data: oi } = await admin
      .from("order_items")
      .select("id")
      .eq("id", production_item_id)
      .eq("tenant_id", tenantId)
      .maybeSingle();
    if (!oi) return apiError("Item de produção inválido", 400);
  }

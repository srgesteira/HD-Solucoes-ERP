...
export function CashFlowPanel({ embedded = false }: CashFlowPanelProps) {
  ...
  }, [embedded, permLoading, access, router]);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/reports/cash-flow?horizon=${horizon}`, {
        credentials: "include",
        cache: "no-store",
      });
  ...
  if (!embedded && (permLoading || !access)) {
    return (
